...
class PPO_discrete:
    ...
    def choose_action(self, real_obs):
        s = obs_norm(real_obs)
        s = torch.unsqueeze(torch.tensor(s, dtype=torch.float), 0).to(self.device)
        with torch.no_grad():
    ...
    def save(self):
        torch.save(self.actor.state_dict(), "model/actor.pkl")
        torch.save(self.critic.state_dict(), "model/critic.pkl")

    def load(self):
        self.actor.load_state_dict(torch.load("model/actor.pkl"))
        self.critic.load_state_dict(torch.load("model/critic.pkl"))

    def save_all(self):
        torch.save(self.actor, 'model_all/actor_all.pkl')